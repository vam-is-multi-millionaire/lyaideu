...
<title>Checkout | LyaiDeu</title><link rel="stylesheet" href="css/style.css">
</head><body class="checkout-body">
<header class="topbar"><nav class="nav"><a class="brand" href="index.php">🛵 Lyai<span>Deu</span></a><a class="btn btn-outline" href="index.php#menu">← Back to Menu</a></nav></header>
<main class="checkout-page container">
  <div class="section-head"><p class="kicker">🧾 Secure checkout</p><h1 class="display">Almost there, <?= htmlspecialchars($user['name']) ?>!</h1><p class="section-sub">Review your items, add delivery details, and place the order.</p></div>
  <?php if ($flash): ?><div class="flash-banner flash-<?= htmlspecialchars($flash['type']) ?>"><?= $flash['msg'] ?></div><?php endif; ?>
  <div id="checkoutEmpty" class="empty-state"><span class="big">🛒</span><p>Your cart is empty.</p><a class="btn btn-primary" href="index.php#menu">Browse Menu</a></div>
  <form id="checkoutForm" action="order_save.php" method="POST" class="checkout-grid">
    ...
    <section class="checkout-card"><h2>📍 Delivery details</h2>
    ...
    <section class="checkout-card"><h2>🛒 Your order</h2><div id="checkoutItems"></div>
      ...
      <button class="btn btn-primary btn-block" type="submit" id="placeOrderBtn">🚀 Place Order</button>
