...
function renderHotels(hotels){const g=$('#hotels-grid');if(!g)return;g.innerHTML=hotels.map(h=>{const logo=esc(h.logo)||'';const kind=(h.kind||'hotel').toLowerCase();const vendor=h.vendor_name&&kind==='hotel'?`<p class="hotel-vendor"><i class="fa-solid fa-store"></i> Kitchen: ${esc(h.vendor_name)}</p><a class="hotel-login" href="vendor"><i class="fa-solid fa-key"></i> Vendor Login</a>`:'';const call=kind==='mart'?`<a class="hotel-call" href="mart"><i class="fa-solid fa-basket-shopping"></i> Shop the Mart</a>`:`<a class="hotel-call" href="tel:+977${esc(h.phone)}"><i class="fa-solid fa-phone"></i> ${esc(h.phone)}</a>`;return `<div class="hotel-card reveal visible" data-kind="${kind}" data-search="${esc((h.name+' '+h.type+' '+storeKindLabel(kind)+' '+(h.hotel||'')+' '+(h.location||'')+' '+(h.vendor_name||'')).toLowerCase())}"><div class="hotel-avatar">${logo?`<img class="hotel-logo" src="${logo}" alt="${esc(h.name)}" loading="lazy">`:faIcon(h.emoji)}</div><div class="hotel-info"><span class="hotel-kind-badge">${storeKindLabel(kind)}</span><h3>${esc(h.name)}</h3><p>${esc(h.type)}</p>${vendor}</div>${call}</div>`}).join('');applyHotelFilters()}
